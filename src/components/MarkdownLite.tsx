...
export default function MarkdownLite({ text }: Props) {
  ...
  return (
    <div className="text-green-200 leading-relaxed whitespace-pre-wrap">
      {blocks.map((b, i) =>
        b.type === "code" ? (
          <pre
            key={i}
            className="my-2 rounded bg-green-950/40 border border-green-500/20 p-2 overflow-x-auto text-xs"
          >
            <code>{b.content}</code>
          </pre>
        ) : (
    ...
    <table className="text-xs min-w-max border-separate border-spacing-0">
      <thead>
        <tr>
          {headers.map((h, hi) => (
            <th
              key={hi}
              className="text-left font-semibold bg-green-950/30 border border-green-500/20 px-2 py-1"
            >
              {renderInline(h)}
            </th>
          ))}
      ...
      <tbody>
        {body.map((r, ri) => (
          <tr key={ri}>
            {r.map((c, ci) => (
              <td key={ci} className="align-top border border-green-500/20 px-2 py-1">
                {renderInline(c)}
              </td>
            ))}
          </tr>
...
function renderInline(input: string): React.ReactNode[] {
  ...
  segments.forEach((seg, idx) => {
    if (/^`[^`]+`$/.test(seg)) {
      ...
      out.push(
        <code key={idx} className="rounded bg-green-950/40 px-1 py-0.5 border border-green-500/20 text-[0.85em]">
          {content}
        </code>
      );
    } else {
      const withBold = splitAndWrap(seg, /\*\*([^*]+)\*\*/g, (m, i) => (
        <strong key={`b-${idx}-${i}`} className="font-semibold text-green-100">{m}</strong>
      ));
      ...
      withBold.forEach((piece, i) => {
        ...
        const italics = splitAndWrap(piece, /(?:\*([^*]+)\*|_([^_]+)_)/g, (m2, ii) => (
          <em key={`i-${idx}-${i}-${ii}`} className="italic text-green-100/90">{m2}</em>
        ));
